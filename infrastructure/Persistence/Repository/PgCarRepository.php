...
{
    public function __construct(private readonly CarDataMapperInterface $mapper)
    {
    }
    ...
    public function save(Car $car): Car
    {
        ...
        try {
            ...
            if (!$record->save()) {
                throw new Exception('Не удалось сохранить объявление: ' . json_encode($record->errors, JSON_UNESCAPED_UNICODE));
            }
            ...
            if ($car->options !== null) {
                ...
                if (!$option->save()) {
                    throw new Exception('Не удалось сохранить характеристики: ' . json_encode($option->errors, JSON_UNESCAPED_UNICODE));
                }
            }

            $transaction->commit();

            $saved = $this->findById((int) $record->id);
            if ($saved === null) {
                throw new Exception('Сохранённое объявление не найдено.');
            }
    ...
    public function findById(int $id): ?Car
    {
        ...
        return $record instanceof CarRecord ? $this->mapper->mapToEntity($record) : null;
    }
    ...
    public function paginate(int $page, int $perPage): array
    {
        ...
        return array_map(fn (CarRecord $record): Car => $this->mapper->mapToEntity($record), $records);
    }
